...
dbconn();
require_once(get_langfile_path("userdetails.php", false, 'chs'));
loggedinorreturn();
parked();
...
// Bind the CNGI account to HUDBT account |->
	function check_code_for_bind($imagehash, $imagestring) {
	...
	?>
	<h2>联盟用户绑定 HUDBT 账号</h2><br /><br />
	<p>当前用户账号： <?php echo $_SERVER["HTTP_USERNAME"]; ?></p>
	<?php
	if(!empty($_POST['username'])) {
		if(!check_code_for_bind($_POST['imagehash'], $_POST['imagestring'])) {
			stdmsg('HUDBT 账号绑定', '认证码输入错误');
			echo '<br /><br />';
		} else {
			...
			if (!$row) {
				...
				echo '<br /><br />';
			} else if($row["passhash"] != md5($row["secret"] . $_POST['password'] . $row["secret"])) {
				stdmsg('HUDBT 账号绑定', 'HUDBT 账号的密码不正确');
				echo '<br /><br />';
			} else if(empty($_SERVER["HTTP_USERNAME"])) {
				stdmsg('HUDBT 账号绑定', '只有 CNGI 用户才能绑定账号');
				echo '<br /><br />';
			}else {
				$res2 = sql_query("UPDATE carsi_users SET hudbt_username='{$_POST['username']}' WHERE carsi_username='{$_SERVER["HTTP_USERNAME"]}'");
				stdmsg('HUDBT 账号绑定', 'HUDBT 账号绑定成功');
				$accountBindSuccsess = true;
			}
		}
	}
	?>
	<?php if(!$accountBindSuccsess): ?>
	<form method="post" action="/userdetails2.php?id=<?php echo GUEST_UID; ?>">
	<table border="0" cellpadding="5">
	<tr><td class="rowhead">HUDBT 账号</td><td class="rowfollow" align="left"><input type="text" name="username" style="width: 180px; border: 1px solid gray" /></td></tr>
	<tr><td class="rowhead">HUDBT 密码</td><td class="rowfollow" align="left"><input type="password" name="password" style="width: 180px; border: 1px solid gray"/></td></tr>
	<?php show_image_code (); ?>
	<tr><td class="toolbox" colspan="2" align="right"><input type="submit" value="绑定" class="btn" /><input type="reset" value="重置" class="btn" /></td></tr>
	</table>
	<?php endif; ?>
	<?php
// End ||
