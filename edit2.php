...
if (!isset($CURUSER) || ($CURUSER["id"] != $row["owner"] && get_user_class() < $torrentmanage_class)) {
	...
	if (get_user_class()>= $torrentsticky_class || (get_user_class() >= $torrentmanage_class && $CURUSER["picker"] == 'yes')){
		...
		if(get_user_class()>=$torrentsticky_class)
		{
			...
			$pickcontent .= "<b>".$lang_edit['text_ie_for'].":&nbsp;</b>"."<input type=\"text\" name=\"promotionuntil\" style=\"width: 120px;\" value=\"". (($row["promotion_time_type"]!=2)? date("Y-m-d H:i:s") :$row["promotion_until"]) . "\" />".$lang_edit['text_promotion_until_note']. "<br />"."&nbsp;&nbsp;&nbsp;";
			// End
