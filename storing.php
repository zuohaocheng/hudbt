...
if(!permissionAuth("storing",$CURUSER['usergroups'],$CURUSER['class'])){
	...
	if($action=="checkin"){
		...
		if(_mysql_num_rows($check_storing_res)==0){
		...
		$check_storing = _mysql_fetch_assoc($check_storing_res);
		...
		if(_mysql_num_rows($snatched_res)==0){
			header("Refresh: 5; url=//$BASEURL/details.php?id=$torrentid");
		...
		$snatched = _mysql_fetch_assoc($snatched_res);
		if($snatched['to_go']!=0){
			header("Refresh: 5; url=//$BASEURL/details.php?id=$torrentid");
	...
	elseif($action=='checkout'){
		...
		if(_mysql_num_rows($exist_res)==0){
			...
			if(_mysql_num_rows($out_seedtime_res)==0){
				header("Refresh: 5; url=//$BASEURL/details.php?id=$torrentid");
			...
			$out_seedtime = _mysql_fetch_assoc($out_seedtime_res);
