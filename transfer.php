...
dbconn();
require_once(get_langfile_path('upload.php', false, 'chs'));
loggedinorreturn();
parked();

$cgbt_tid = (int) $_GET['cgbt_tid'];
$cgbt_info = get_cgbt_detail($cgbt_tid);
...
?>
	<form id="compose" enctype="multipart/form-data" action="taketransfer.php" method="post" name="upload">
			<?php
			...
			<table border="1" cellspacing="0" cellpadding="5" width="940">
				...
				tr("旧平台种子 ID", "<a href=\"/old/details.php?tid={$cgbt_tid}\" target=\"_blank\" title=\"点我在新页面中查看\">{$cgbt_tid}</a><input type=\"hidden\" name=\"cgbt_tid\" value=\"{$cgbt_tid}\"/><input type=\"hidden\" name=\"filename\" value=\"{$cgbt_info['filename']}\"/>\n", 1);
				tr($lang_upload['row_torrent_name'], "<input type=\"text\" style=\"width: 650px;\" id=\"name\" name=\"name\" value=\"".filterFileName($cgbt_info['title'])."\" /><br /><font class=\"medium\">".$lang_upload['text_torrent_name_note']."</font>", 1);
				...
				function filterFileName($title) {
					$title = preg_replace('/([0-9]+)\.([0-9]+)/i', '$1____$2', $title);
					$title = preg_replace('/([0-9]+)\.([0-9]+)/i', '$1____$2', $title);
					$title = str_replace('.', ' ', $title);
					$title = str_replace('____', '.', $title);
					return $title;
				}
				get_external_tr();
				...
				// Modified by BruceWolf
				// Get the describetion of CGBT
				textbbcode("upload","descr","{$cgbt_info['content']}",false);
				print("</td></tr>\n");
...
function get_cgbt_detail($cgbt_tid)
{
	if(empty($cgbt_tid)) {
	...
	$sql = "SELECT `save_as` as `title`, `descr` as `content`, `filename` FROM torrents where id='{$cgbt_tid}' limit 0,1";
	//echo $sql;
