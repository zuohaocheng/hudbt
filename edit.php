...
if (!$id) {
  ...
  stderr($lang_edit['text_cannot_edit_torrent'], $lang_edit['text_cannot_edit_torrent_note'], false);
}

$row = sql_query("SELECT torrents.*, categories.mode as cat_mode FROM torrents LEFT JOIN categories ON category = categories.id WHERE torrents.id = ?", [$id])->fetch();
if (!$row) {
