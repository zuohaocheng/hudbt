...
<table class="head" cellspacing="0" cellpadding="0" align="center">
	<tr>
	    <td class="clear" style="width:200px"><!-- Modified by bluemonster 20111122,style --> 
<?php
if ($logo_main == "")
{
?>
...
function format_comment2($text, $strip_html = true, $xssclean = false, $newtab = false, $imageresizer = true, $image_max_width = 700, $enableimage = true, $enableflash = true , $imagenum = -1, $image_max_height = 0, $adid = 0)
{
	global $lang_functions;
	global $CURUSER, $SITENAME, $BASEURL, $enableattach_attachment;
	global $tempCode, $tempCodeCount;
	...
	$s = preg_replace("/\[img=(\d{1,4})[x|\,](\d{1,4})\]\s*([^\[\<\r\n]+?)\s*\[\/img\]/ies", "bbcodeurl('\\3', '<img width=\"\\1\" height=\"\\2\" src=\"%s\" border=\"0\" alt=\"\" />')", $s);
	$searcharray['bbcode_regexp'] = array(
		...
		$searcharray['bbcode_regexp'][] = "/\[table(?:=(\d{1,4}%?)(?:,([\(\)%,#\w ]+))?)?\]\s*(.+?)\s*\[\/table\]/ies";
		$replacearray['bbcode_regexp'][] = "parsetable('\\1', '\\2', '\\3')";
		$searcharray['bbcode_regexp'][] = "/\[table(?:=(\d{1,4}%?)(?:,([\(\)%,#\w ]+))?)?\]\s*(.+?)\s*\[\/table\]/ies";
		$replacearray['bbcode_regexp'][] = "parsetable('\\1', '\\2', '\\3')";

		$searcharray['bbcode_regexp'][] = "/\s*\[quote\][\n\r]*(.+?)[\n\r]*\[\/quote\]\s*/is";
		$replacearray['bbcode_regexp'][] = "<br><br><div class=\"msgbody\"><div class=\"msgheader\">QUOTE:</div><div class=\"msgborder\">\\1</div></div><br>";
	...
	@$s = preg_replace( $searcharray['bbcode_regexp'] , $replacearray['bbcode_regexp'] , $s );
	$s = str_replace( $searcharray['bbcode_str'], $replacearray['bbcode_str'], $s);
// �ᵼ�¶Ա�ǩ�� url ��Ϊֵ����������?ȥ��
//	$s = format_urls($s, $newtab);
...
function get_torrent_promotion_append_sub($promotion = 1,$forcemode = "",$showtimeleft = false, $added = "", $promotionTimeType = 0, $promotionUntil = ''){
	...
	switch ($promotion){
		...
		case 4:
		{
			if ($showtimeleft && (($expiretwoupfree_torrent && $promotionTimeType == 0) || $promotionTimeType == 2))
			{
				...
				$timeout = gettime(date("Y-m-d H:i:s", $futuretime), false, false, true, false, true);
				if ($timeout)
				$onmouseover = "<font color='#00CC66'>".$lang_functions['text_will_end_in'].$timeout."</font>";
				else $promotion = 1;
			}
			break;
		}
	...
	if (($CURUSER['appendpromotion'] == 'word' && $forcemode == "" ) || $forcemode == 'word'){
		if(($promotion==2 && get_global_sp_state() == 1) || get_global_sp_state() == 2){
//			$sp_torrent = " [<font class='free' ".$onmouseover."</font>]";
			$sp_torrent = "[".$onmouseover."]";
		}
		
		elseif(($promotion==4 && get_global_sp_state() == 1) || get_global_sp_state() == 4){
//			$sp_torrent = " [<font class='twoupfree' ".$onmouseover."</font>]";
			$sp_torrent = "[".$onmouseover."]";
		}
	...
	elseif (($CURUSER['appendpromotion'] == 'icon' && $forcemode == "") || $forcemode == 'icon'){
		if(($promotion==2 && get_global_sp_state() == 1) || get_global_sp_state() == 2)
//			$sp_torrent = $onmouseover ? "[".$onmouseover."]" : "title=\"".$lang_functions['text_free']."\"";
			$sp_torrent = $onmouseover ? "[".$onmouseover."]" : "[<font color='#0000FF'>".$lang_functions['text_will_end_in'].$lang_functions['text_forever']."</font>]";

		elseif(($promotion==4 && get_global_sp_state() == 1) || get_global_sp_state() == 4)
//			$sp_torrent = $onmouseover ? "[".$onmouseover."]" : "title=\"".$lang_functions['text_free_two_times_up']."\"";
			$sp_torrent = $onmouseover ? "[".$onmouseover."]" : "[<font color='#0000FF'>".$lang_functions['text_will_end_in'].$lang_functions['text_forever']."</font>]";
	}
	*/
